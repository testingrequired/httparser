...
impl ParserConfig {
    ...
    /// let mut response = httparser::Response::new(&mut headers);
    ///
    /// let res = httparser::ParserConfig::default()
    ///     .allow_obsolete_multiline_headers_in_responses(true)
    ///     .parse_response(&mut response, buf);
    ///
    /// assert_eq!(res, Ok(httparser::Status::Complete(buf.len())));
    ///
    ...
    /// let mut response = httparser::Response::new(&mut headers[..]);
    /// let result = httparser::ParserConfig::default()
    ///     .allow_space_before_first_header_name(true)
    ///     .parse_response(&mut response, buf);
    ///
    /// assert_eq!(result, Ok(httparser::Status::Complete(buf.len())));
...
///
/// ```
/// let headers = [httparser::EMPTY_HEADER; 64];
/// ```
...
/// let mut headers = [httparser::EMPTY_HEADER; 4];
/// assert_eq!(httparser::parse_headers(buf, &mut headers),
///            Ok(httparser::Status::Complete((27, &[
///                httparser::Header { name: "Host", value: b"foo.bar" },
